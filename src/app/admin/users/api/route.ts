...
async function audit(actorId: string, action: string, targetUserId?: string, meta?: any) {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (!tempPassword || tempPassword.length < 6) return jsonError("Temp password must be 6+ chars");
      ...
      if (createErr || !created.user) return jsonError(createErr?.message || "Failed to create user", 400);
      ...
      await audit(actor.id, "CREATE_ADMIN", created.user.id, { role, phone });
    ...
    if (action === "update") {
      ...
      const { error } = await supabaseAdmin.from("admin_users").update(patch).eq("user_id", user_id);
      if (error) return jsonError(error.message, 400);

      await audit(actor.id, "UPDATE_ADMIN", user_id, patch);
      return NextResponse.json({ ok: true });
    }
    ...
    if (action === "delete") {
      ...
      const { error: delRowErr } = await supabaseAdmin.from("admin_users").delete().eq("user_id", user_id);
